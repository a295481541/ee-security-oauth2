...
public class EndUserSignOnBizImpl implements EndUserSignOnBizService {
	...
	public AccessToken getAccessToken(String appId, String secretKey, String grantCode) {
		...
		SimpleResponse rmCodeResult = 
				getSignOnUtil().removeCodeOrToken(SecurityCacheKey.ENDUSER_GRANTCODE_PREFIX, grantCode, appId);
		...
		/* 标记最终用户已缓存令牌 */
		getSignOnUtil().markUserTokenInApp(SecurityCacheKey.ENDUSER_CACHED_TOKEN, appId, userId, 
	...
	public AccessToken refreshAccessToken(String appId, String secretKey, String refreshToken, String endUserId) {
		...
		/* 更新最终用户已缓存令牌 */
		getSignOnUtil().markUserTokenInApp(SecurityCacheKey.ENDUSER_CACHED_TOKEN, appId, userId, seriesId, mkAccessTokenResult.getResult(), refreshToken);
...
//				getSignOnUtil().makeGrantCode(AuthenCacheKey.ENDUSER_GRANTCODE_PREFIX, appId, endUser.getAtid()+":"+app.getBusinessSeries().getAtid());
		StringResponse makeCodeResult = 
				getSignOnUtil().makeGrantCode(SecurityCacheKey.ENDUSER_GRANTCODE_PREFIX, appId, endUser.getAtid(),app.getBusinessSeries().getAtid());
