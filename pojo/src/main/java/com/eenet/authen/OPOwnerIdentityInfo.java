 ...
 * @author Orion
 */
public class OPOwnerIdentityInfo {
	private static ThreadLocal<String> accessToken = new ThreadLocal<String>();//用户访问令牌
	private static ThreadLocal<String> appSecretKey = new ThreadLocal<String>();//app秘钥（加密）
	private static ThreadLocal<String> redirectURI = new ThreadLocal<String>();//app跳转地址（仅web应用有效）
	private static ThreadLocal<String> userType = new ThreadLocal<String>();//用户类型（取值：endUser,adminUser,anonymous）
	/**
	 * @return the 用户访问令牌
	 */
	...
	public static void setUsertype(String usertype) {
		if ("endUser".equals(usertype) || "adminUser".equals(usertype) || "anonymous".equals(usertype))
			...
			throw new RuntimeException("用户类型只允许：endUser,adminUser,anonymous("+OPOwnerIdentityInfo.class.getName()+")");
	}
